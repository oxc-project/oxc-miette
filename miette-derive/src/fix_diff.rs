...
impl Parse for FixDiff {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        if ident == "fix_diff" {
            ...
            Err(syn::Error::new(ident.span(), "not a fix_diff"))
        }
    }
}
...
impl FixDiff {
    ...
    fn from_fields_vec(fields: Vec<&syn::Field>) -> syn::Result<Option<Self>> {
        for (i, field) in fields.iter().enumerate() {
            for attr in &field.attrs {
                if attr.path().is_ident("fix_diff") {
                    let fix_diff = if let Some(ident) = field.ident.clone() {
                        syn::Member::Named(ident)
                    } else {
                        syn::Member::Unnamed(syn::Index { index: i as u32, span: field.span() })
                    };
                    return Ok(Some(FixDiff::Field(fix_diff, Box::new(field.ty.clone()))));
                }
            }
        }
        Ok(None)
    ...
    pub(crate) fn gen_enum(variants: &[DiagnosticDef]) -> Option<TokenStream> {
        gen_all_variants_with(
            variants,
            WhichFn::FixDiff,
            |ident, fields, DiagnosticConcreteArgs { fix_diff, .. }| {
                let (display_pat, display_members) = display_pat_members(fields);
                match &fix_diff.as_ref()? {
                    FixDiff::Display(display) => {
                        let (fmt, args) = display.expand_shorthand_cloned(&display_members);
                        Some(quote! {
                            Self::#ident #display_pat => std::option::Option::Some(std::boxed::Box::new(format!(#fmt #args))),
                        })
                    }
                    FixDiff::Field(member, ty) => {
                        let fix_diff = match &member {
                        ...
                        Some(quote! {
                            Self::#ident #display_pat => {
                                ...
                                miette::macro_helpers::OptionalWrapper::<#ty>::new().to_option(&#fix_diff).as_ref().map(|#var| -> std::boxed::Box<dyn std::fmt::Display + '_> { std::boxed::Box::new(format!("{}", #var)) })
                            },
                        })
                    }
                }
